	...
	public partial class ModbusClient
	{
		...
		public void Connect()
		{
            ...
            if (!udpFlag)
            {
                ...
                if (!success)
                {
                    throw new modbus.exceptions.ConnectionException("connection timed out");
                }
        ...
        /// <summary>
        /// Converts two ModbusRegisters to Float - Example: bullxitModbus.ModbusClient.ConvertRegistersToFloat(modbusClient.ReadHoldingRegisters(19,2))
        /// </summary>
        ...
        /// <summary>
        /// Converts float to two ModbusRegisters - Example:  modbusClient.WriteMultipleRegisters(24, bullxitModbus.ModbusClient.ConvertFloatToTwoRegisters((float)1.22));
        /// </summary>
		...
		public bool[] ReadDiscreteInputs(int startingAddress, int quantity)
		{
            ...
            if (serialport != null)
                if (!serialport.IsOpen)
            	{
                    ...
                    throw new modbus.exceptions.SerialPortNotOpenedException("serial port not opened");
            	}
			if (tcpClient == null & !udpFlag & serialport==null)
			{
                ...
                throw new modbus.exceptions.ConnectionException("connection error");
			}
            ...
            if (data[7] == 0x82 & data[8] == 0x01)
            {
                ...
                throw new modbus.exceptions.FunctionCodeNotSupportedException("Function code not supported by master");
            }
            if (data[7] == 0x82 & data[8] == 0x02)
            {
                ...
                throw new modbus.exceptions.StartingAddressInvalidException("Starting address invalid or starting address + quantity invalid");
            }
            if (data[7] == 0x82 & data[8] == 0x03)
            {
            	if (debug) StoreLogData.Instance.Store("QuantityInvalidException Throwed", System.DateTime.Now);
                throw new modbus.exceptions.QuantityInvalidException("quantity invalid");
            }
            if (data[7] == 0x82 & data[8] == 0x04)
            {
            	if (debug) StoreLogData.Instance.Store("ModbusException Throwed", System.DateTime.Now);
                throw new modbus.exceptions.ModbusException("error reading");
            }
            if (serialport != null)
            {
            crc = BitConverter.GetBytes(calculateCRC(data, (ushort)(data[8]+3), 6));
                if ((crc[0] != data[data[8] + 9] | crc[1] != data[data[8] + 10]) & dataReceived)
                {
                    ...
                    throw new modbus.exceptions.CRCCheckFailedException("Response CRC check failed");
                }
        ...
        /// <summary>
        /// Convert binary representation "100101001" to byte array
        /// </summary>
        /// <param name="binary">Binary representation "10101011". Multiple of 8</param>
        /// <returns>a byte array</returns>
        public static byte[] BinaryToDecimal(string binary)
        {
